...
class OzioGalleryViewPicasa extends JViewLegacy
{
	...
	function get_allowed_albums($menu_id){
		...
		if (strpos($item->link, "&view=00fuerte") === false && strpos($item->link, "&view=lightgallery") === false && strpos($item->link, "&view=nano") === false && strpos($item->link, "&view=jgallery") === false){
			//invalid id
		}else if (strpos($item->link, "&view=lightgallery") !== false && $item->getParams()->get("source_kind", "photo")!=='photo'){
			//invalid id
		}else if (strpos($item->link, "&view=00fuerte") !== false || strpos($item->link, "&view=lightgallery") !== false){
			
			$params = $item->getParams()->toArray();
			...
			$white_list = array();
			$black_list = array();
			$wlist = explode('|',$item->getParams()->get("ozio_nano_whiteList", ""));
			$blist = explode('|',$item->getParams()->get("ozio_nano_blackList", ""));
			foreach ($wlist as $k){
				$k = trim($k);
				if (!empty($k)){
					$white_list[] = $k;
			...
			if ($kind=='picasa' && $albumvisibility=='limited'){
				$allowed_albums[] = $item->getParams()->get("limitedalbum", "");
			}else if ($kind=='picasa'){
	...
	function refresh_token($credentials){
		...
		if (isset($resp['access_token'])){
			...
			JFactory::getApplication()->enqueueMessage("Unable to refresh token ".$response);
			die();
		}		
