	...
	<tr>
		<td valign="top">
			...
			<fieldset class="adminform">
				<legend><?php echo JText::_( 'COM_OZIOGALLERY4_STATISTICHE' ); ?></legend>
				<?php 
				...
				for ($i=0, $n; $i < $n; $i++) {
					...
					if ( $row->link == 'index.php?option=com_oziogallery4&view=00fuerte'){
						$row->params=json_decode($row->params,true);
						$row->skin='00fuerte';
						$row->link='index.php?option=com_oziogallery4&view=00fuerte&Itemid='.$row->id;
						$g_parameters[]=$row;
					}
					if ( $row->link == 'index.php?option=com_oziogallery4&view=lightgallery'){
						...
						if ($result->get("source_kind", "photo")=='photo'){
							...
							$row->link='index.php?option=com_oziogallery4&view=lightgallery&Itemid='.$row->id;
							$g_parameters[]=$row;
						}
					}
					if ( $row->link == 'index.php?option=com_oziogallery4&view=jgallery'){
						$result = new JRegistry;
						$result->loadString($row->params);
						...
						if ($albumvisibility=='limited'){
							...
							$row->link='index.php?option=com_oziogallery4&view=nano&Itemid='.$row->id.$deeplink;

							$g_parameters[]=$row;
						}
					}
					if ( $row->link == 'index.php?option=com_oziogallery4&view=nano'){
						$result = new JRegistry;
						$result->loadString($row->params);
				...
				for ($i=0, $n; $i < $n; $i++) {
					...
					if ( $row->link == 'index.php?option=com_oziogallery4&view=nano'){
						...
						if ($kind=='picasa' && $albumvisibility=='limited'){
							...
							$link = 'index.php?option=com_oziogallery4&view=nano&Itemid='.$item->id;
							?>
									//nano
				...
				<table id="first10albums" class="table">
					...
					<p><?php echo JText::_('COM_OZIOGALLERY4_FIRST_10_ALBUMS');	?></p>
					<thead>
						<tr>
				...
				<table id="first10photos" class="table">
					...
					<p><?php echo JText::_('COM_OZIOGALLERY4_FIRST_10_PHOTOS');	?></p>
					<thead>
						<tr>
