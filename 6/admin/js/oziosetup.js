jQuery(document).ready(function ($)
{
	...
	function ozio_credentials_table_setup_action($td, item){
		...
		if (item.status=='pending'){
			...
			*/
			//var $auth_btn = $('<iframe src="index.php?option=com_oziogallery4&view=setup_auth&tmpl=component&credentials_id='+item.id+'"></iframe>');
			...
			//
			//
				$('#ozio_auth_modal .modal-body').append('<iframe src="index.php?option=com_oziogallery4&view=setup_auth&tmpl=component&credentials_id='+item.id+'" style="margin:0;padding:0;border:0;width:100%;height:250px;overflow:hidden;"></iframe>');
				$('#ozio_auth_modal').modal("show");
			});
	...
	$('#ozio_credentials_add').on('click',function(){
		...
		$('#ozio_cerentials_modal').modal("show");
		
		
	});
	$('.ozio-close-modal').on('click',function(){
