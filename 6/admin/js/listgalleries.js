...
function LoadAlbums()
{
	var input = document.getElementById('jform_params_userid');
...
function loadpagination(username, start_slide,next_token){
		jQuery("#album_selection").pwi(
			{
				picasaUrl: 'index.php?option=com_oziogallery4&view=picasa&format=raw',
				mode:'user_albums',
				username:username,
...
function ozio_addalbums(albums){
	...
	tabella['NONE'] = '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_NONE"); ?>';
	tabella['PROFILEPHOTOS'] = '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_PROFILEPHOTOS"); ?>';
	tabella['SCRAPBOOK'] = '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_SCRAPBOOK"); ?>';
	tabella['BUZZ'] = '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_BUZZ"); ?>';
	...
	for (var a = 0; a < albums.length; ++a)
	{
		...
		//if (title == '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_BUZZ"); ?>')
		//{
	...
	//addoption(select, "posts", '<?php echo JText::_("COM_OZIOGALLERY4_ALBUMTYPE_BUZZ"); ?>' + ' (' + numbuzz + ')');
...
try{
    ...
    jQuery(document).ready(function(){
        ...
        OnFixedHeightChange();
        OnMarkersIconChange();
        OnLightGallerySourceKindChange();
        OnUseridChange();
        
        	var input = document.getElementById('jform_params_userid');

		// Evento onChange
		input.addEventListener('realChange', OnUseridChange);
		input.addEventListener('paste', OnUseridPaste);
		input.addEventListener('cut', OnUseridCut);
		input.addEventListener('change', OnUseridExit);
    })
